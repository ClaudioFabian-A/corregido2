...
router.get('/', async (req, res) => {
    ...
    limit ? res.status(206).json(part) : res.status(200).json(products)
...
router.put('/:pid', async (req, res) => {
    ...
    product ? res.status(400).json({ messege: "Prducto no agregado" }) : res.status(200).json({ message: "producto actualizado" })
